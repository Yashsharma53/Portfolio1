...
// Bold ink-brush style crow silhouettes — single solid path, ragged wing edges
// Matching Itachi's anime crows: pure black, simple, graphic
const CROW_PATHS = [
  // Wings spread wide — gliding
  ...
  // Compact — wings slightly back, cruising
...
// Small floating feather pieces
const FEATHER_PATHS = [
  "M0 5 C2 3 4 1 6 0 C5 2 4 4 3 6 C2 5 1 5 0 5 Z",
  "M0 3 C1 1 3 0 5 0 C4 2 3 3 2 5 C1 4 0 4 0 3 Z",
  "M0 0 C2 1 3 3 4 5 C2 4 1 3 0 3 C0 2 0 1 0 0 Z",
];
...
function generateBgCrows(): BgCrow[] {
  const crows: BgCrow[] = [];
...
function generateFeathers(): BgFeather[] {
  const feathers: BgFeather[] = [];
  for (let i = 0; i < FEATHER_COUNT; i++) {
    feathers.push({
      id: i,
      variant: Math.floor(Math.random() * FEATHER_PATHS.length),
      y: 5 + Math.random() * 85,
...
export default function CrowBackground() {
  ...
  const [crows] = useState<BgCrow[]>(() => generateBgCrows());
  const [feathers] = useState<BgFeather[]>(() => generateFeathers());
  ...
  // Animate crows and feathers
  useEffect(() => {
    if (!isLight || !containerRef.current) return;

    const crowEls = containerRef.current.querySelectorAll<HTMLElement>(".bg-crow");
    const featherEls = containerRef.current.querySelectorAll<HTMLElement>(".bg-feather");
    const allEls = [...Array.from(crowEls), ...Array.from(featherEls)];

    crowEls.forEach((el, i) => {
      const crow = crows[i];
      if (!crow) return;

      const startX = crow.flipX ? window.innerWidth + 120 : -120;
      const endX = crow.flipX ? -200 : window.innerWidth + 200;
      const duration = window.innerWidth / crow.speed;

      gsap.set(el, {
        x: startX,
        y: (crow.y / 100) * window.innerHeight,
      ...
      gsap.to(el, {
        ...
        ease: "none",
        repeat: -1,
        delay: (i / CROW_COUNT) * duration,
        onRepeat: () => {
          gsap.set(el, {
            x: startX,
            y: (crow.y / 100) * window.innerHeight + (Math.random() - 0.5) * 120,
          });
        },
      });
    ...
    featherEls.forEach((el, i) => {
      const f = feathers[i];
      if (!f) return;

      const startX = Math.random() > 0.5 ? window.innerWidth + 50 : -50;
      const endX = startX > 0 ? -100 : window.innerWidth + 100;
      const duration = window.innerWidth / f.speed;
      ...
      gsap.to(el, {
        x: endX,
        rotation: f.rotation + 360 + Math.random() * 360,
        duration,
        ease: "none",
        repeat: -1,
        ...
        onRepeat: () => {
          gsap.set(el, {
            x: startX,
            y: (f.y / 100) * window.innerHeight + (Math.random() - 0.5) * 150,
          });
        },
      });
    ...
    >
      ...
      {feathers.map((f) => (
        ...
        >
          ...
          >
            <path d={FEATHER_PATHS[f.variant]} fill="#000" />
          </svg>
        </div>
      ))}
    </div>
